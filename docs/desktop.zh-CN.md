...
Review 面板在 Desktop 浏览器存储中记住最近选择的来源；若该来源需要当前不存在的线程，则回退到未暂存改动。GitHub PR 可附加到聊天或从聊天解除附加；Desktop 在浏览器存储中为每个线程最多保存 20 个经过校验的 PR 链接，并在 Git 设置允许时于侧边栏显示附加图标。Git 设置仍保存在 Server 配置中。Server 在共享审计日志中记录 Git 修改操作的开始及结果，并关联请求 ID；审计事件不包含路径、提交消息、PR 正文或文件内容。
