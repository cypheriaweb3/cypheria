...
对于有本地工作目录的会话，Codex 的 Review 面板通过 Server Git API 读取仓库。它展示已暂存及未暂存的文件和文本差异，并提供仓库初始化、分支创建与切换、文件暂存、取消暂存、提交、推送，以及托管工作树的创建和删除操作。删除工作树要求其没有未提交改动，并保存已提交的 HEAD，供通过 API 恢复。面板优先使用会话工作目录，并在打开时刷新本地状态。
