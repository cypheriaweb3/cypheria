...
General 设置在 General 区域上方展示 Permissions。Default permissions 始终打开且不持久化。Full access 的可用性保存在 Desktop `config.json` 的 `composer.permissionModeVisibility`；开启前须确认，且只会让 composer 菜单显示 Full access。Codex 权限和其他默认值通过 App Server 保存在隔离的 Codex `config.toml` 中。
