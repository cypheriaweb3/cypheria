...
CLI 路径还能读取审查线程、在变更行发表行内评论、回复线程，并在账户有权限时解决或重新打开线程。读写前会核对面板显示的 PR head，修改线程前还会检查该线程属于当前 PR。如果 GitHub 上有超过 100 个线程或某个线程有超过 100 条回复，面板会标明讨论不完整。
