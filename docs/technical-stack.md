...
| Claude Code protocol | Network-safe `@anthropic-ai/claude-agent-sdk@0.3.270` surface under `agent.claude.*` |
| Pi RPC protocol | `@earendil-works/pi-coding-agent@0.85.1` `pi --mode rpc` surface under `agent.pi.*` |
...
`@cypheria/protocol` authors live WebSocket contracts with Zod and owns the generated Codex App Server TypeScript, JSON Schemas, response mappings, and validators. Its complete `agent.codex.*` RPC and notification catalog is mechanically derived from those committed artifacts; provider payloads stay JSON-transparent on the shared wire. Cypheria-owned envelopes use stripping objects, while provider-owned extension surfaces are explicitly validated. Final large message unions are explicitly AOT-compiled, and nested discriminated unions expose every routable agent message `type`. Object schemas are composed with `.extend()` or shape spread; generated intersections are retained only when an upstream JSON Schema combines a variant union with common constraints. It also exposes `agent.acp.<operation>.request|response|notification` messages backed by the official SDK's stable-v1 and explicit draft-v2 types and generated Zod validators; numeric `protocolVersion` is the ACP version discriminator. Underscore-prefixed extension methods and v2 batches have dedicated types, and batch entries use another nested discriminated union. A generated ACP catalog derives 51 RPC pairs and 22 notifications from the SDK handler declarations, and every known method returns the canonical output of its selected params or result validator. A minimal pinned pnpm patch exposes the SDK's shipped-but-private v1/v2 Zod modules without copying them.

The same package pins `@anthropic-ai/claude-agent-sdk@0.3.270` and exposes its selected network-safe API through `agent.claude.*`. Serializable `query()` options, session/settings functions, all `Query` control methods, streaming input, and 40 output variants are represented without sending callbacks, iterators, process handles, or session-store objects across the wire. `startup()`, `tool()`, and `createSdkMcpServer()` are explicit exclusions. A generated registry inspects the installed declarations and fails lifecycle checks if the SDK's `SDKMessage` union, 29 `Query` methods, 67 option keys, 17 exported functions, or package version drift. Runtime SDK types are available through `@cypheria/protocol/claude-types`; `@cypheria/client/claude` supplies the SDK-shaped client facade, and the server owns one SDK runtime per logical session. See [Claude Agent SDK Protocol](claude-agent-sdk-protocol.md).

Protocol also pins `@earendil-works/pi-coding-agent@0.85.1`. Its full 33-command `pi --mode rpc` API is mapped to paired, directly discriminable `agent.pi.*.request|response` messages; 23 agent/session events, extension errors, and all nine extension UI operations retain their native typed payload under concrete notifications or reverse RPCs. `@cypheria/protocol/pi-types` provides type-only upstream access, while conversion helpers prepare exact JSONL values for the server adapter. `@cypheria/client/pi` rebuilds the process-independent `RpcClient` command/event API over a borrowed connection. The server owns one `pi --mode rpc` process per logical session. See [Pi RPC Protocol](pi-rpc-protocol.md).
...
`@cypheria/client` depends on `@cypheria/protocol`, the transport-only `@cypheria/relay`, and the same exact `@agentclientprotocol/sdk` release used by protocol. Its `ServerClient` implements an injectable transport boundary, browser and Node WebSocket adapters, hello/authentication, correlation, timeouts, protocol validation, typed errors, event delivery, and bounded exponential reconnects. `CypheriaApi` is a lifecycle-free borrowed facade; `CypheriaClient` owns one connection. The facade exposes only current protocol message families: server status/diagnostics/configuration and typed Codex, ACP, and Claude traffic. It does not infer product actions from runtime method strings. `@cypheria/client/codex` implements Cypheria-owned SDK-shaped `client()` and `ClientApp` APIs: `connect()` and `connectWith()` accept `CypheriaApi`, select `agent.codex` internally, and expose a typed async `ClientContext.request()` for each request/response exchange. Fluent handlers receive notifications and reverse requests and write typed reverse responses without a generated action tree. Stable `@cypheria/client/acp` and explicit `@cypheria/client/acp/v2` entries use the same `CypheriaApi` connection shape, selectively re-export supported upstream helpers and types, and omit deprecated connection APIs. Their private directional Web Streams bridge translates SDK JSON-RPC traffic to protocol-owned logical messages, preserves v2 batches through the dedicated nested batch message, uses connection-local request-ID state to restore response types, and tears down pending ACP work when the Cypheria transport disconnects. `@cypheria/client/claude` binds a borrowed `CypheriaApi` into an SDK-shaped facade whose `query()` is an async iterator, whose control/session/settings methods retain upstream call shapes, and whose local abort and transport failures terminate only the corresponding remote query. Codex apps likewise allow one active consumer per endpoint and close on transport loss.
