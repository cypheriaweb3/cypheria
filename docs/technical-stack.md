...
The same package pins `@anthropic-ai/claude-agent-sdk@0.3.270` and exposes its selected network-safe API through `agent.claude.*`. Serializable `query()` options, session/settings functions, all `Query` control methods, streaming input, and 40 output variants are represented without sending callbacks, iterators, process handles, or session-store objects across the wire. `startup()`, `tool()`, and `createSdkMcpServer()` are explicit exclusions. A generated registry inspects the installed declarations and fails lifecycle checks if the SDK's `SDKMessage` union, 29 `Query` methods, 67 option keys, 17 exported functions, or package version drift. Runtime SDK types are available through `@cypheria/protocol/claude-types`; `@cypheria/client/claude` supplies the SDK-shaped client facade, while server dispatch remains deferred. See [Claude Agent SDK Protocol](claude-agent-sdk-protocol.md).

Protocol also pins `@earendil-works/pi-coding-agent@0.85.1`. Its full 33-command `pi --mode rpc` API is mapped to paired, directly discriminable `agent.pi.*.request|response` messages; 23 agent/session events, extension errors, and all nine extension UI operations retain their native typed payload under concrete notifications or reverse RPCs. `@cypheria/protocol/pi-types` provides type-only upstream access, while conversion helpers prepare exact JSONL values for the future server adapter. `@cypheria/client/pi` rebuilds the process-independent `RpcClient` command/event API over a borrowed connection. Child-process start/stop, stderr, executable, environment, and signal ownership stay server-side. See [Pi RPC Protocol](pi-rpc-protocol.md).
