...
| Internal provider adapters | Codex, Claude Agent SDK, Pi RPC, OpenCode SDK, and ACP |
| OpenCode | Registry binary `1.18.30`, `@opencode-ai/sdk@1.18.31`, shared loopback server |
| Managed agent toolchains | Latest stable Node LTS, uv-managed stable CPython, uv/uvx |
...
`@cypheria/client` depends on `@cypheria/protocol` and the transport-only `@cypheria/relay`. Its `ServerClient` implements an injectable transport boundary, browser and Node WebSocket adapters, hello/authentication, correlation, timeouts, protocol validation, typed errors, event delivery, and bounded reconnects. `CypheriaApi` is a lifecycle-free borrowed facade; `CypheriaClient` owns one connection. The preferred facades are `agents`, `projects`, `sections`, `threads`, `timeline`, and `schedules`; singular `agent`/`thread` and grouped `projectThread` remain compatibility aliases during client migration. Provider-specific client subpaths remain excluded; all clients observe the same server-owned Thread state and notifications.

Schedules are a server service rather than a desktop timer. SQLite stores the cadence, target, next slot, optimistic revision, lease, and independent run records. The server advances a claimed slot and creates its run in one transaction before dispatching a new Thread, an existing Thread turn, or a bounded Web3 runtime method. Startup marks unfinished runs interrupted and clears their leases without rewinding the schedule, which prevents automatic replay of an interrupted signature or transaction submission. The cron implementation uses five fields and IANA time zones; missed interval and cron slots advance to the next future occurrence instead of creating an unbounded backlog.

Correlated facade calls share `{ signal, timeoutMs }` controls and cancel lazy sends when their
deadline wins. Required server capabilities are checked against logical `server.status.notification` before transport
writes. Codex connections complete an explicit per-client initialize/initialized state machine,
and reverse RPC failures are reported through the local handler-error hook.

`@cypheria/client` also accepts a mutually exclusive `relayOffer` transport configuration. It uses
`@cypheria/relay` to complete E2EE before the top-level Cypheria `hello`. `apps/relay`
...
Desktop offers a searchable Codex-style language picker. Its preference is persisted in Electron-local `$CYPHERIA_HOME/desktop/desktop-settings.json` alongside appearance and workspace-layout settings, independently of Codex configuration. Electron resolves automatic detection from its preferred-language list, passes both the preference and resolved catalog locale through preload bootstrap, and broadcasts later changes through typed IPC. Because the packaged SPA shell is prerendered at build time, both server output and the first client render use the English source catalog; immediately after hydration the renderer activates the bootstrapped catalog, updates the document `lang` and `dir` attributes, and changes language without reloading. This avoids locale-dependent hydration mismatches. English and Simplified Chinese have bundled catalogs; all other picker values currently resolve to English while retaining their original selection. PO files are committed; `pnpm --filter @cypheria/desktop i18n:extract` updates them and `i18n:compile` validates that translations are complete.

Settings navigation is grouped into Personal, Integrations, Coding, and Archived sections and filters by localized destination name. `Cmd/Ctrl+F` focuses that search from any settings route. The Archived chats route pages `thread/list` with `archived: true`, passes its query to the Cypheria Server, and invokes `thread/unarchive` plus confirmed `thread/delete` through `@cypheria/client`. ChatGPT-service settings without a Codex App Server or Cypheria-owned backend remain out of scope instead of being presented as inert controls.

Electron user data is rooted under `$CYPHERIA_HOME/desktop`, while Chromium session data remains isolated under `$CYPHERIA_HOME/browser`. Desktop-only appearance, language, layout, server-manager, window, sound, and update preferences share the versioned `desktop-settings.json` document and use serialized atomic writes.

## Codex Integration

The server owns Codex for every client. Desktop live turns, durable history, account/login, model discovery and defaults, permission defaults and profiles, plugins, skills, MCP servers, marketplaces, Codex Apps, auto-review notifications, reverse-request interactions, and project terminals enter through unified Cypheria client APIs. Shared Codex model and permission defaults are stored under `agents.codex` in `$CYPHERIA_HOME/config/config.json` and projected into the managed Codex runtime:
...
The Server Codex adapter owns Cypheria's Codex integration. It:
...
- AI SDK tool definitions are not treated as App Server dynamic-tool callbacks. Server services register experimental dynamic-tool schemas and handlers; schemas are sent in `thread/start` and `item/tool/call` is dispatched to the registered handler.
...
`apps/server` owns the `codex app-server` child process. It starts Codex with `CODEX_HOME=$CYPHERIA_HOME/codex`, validates the generated protocol version, waits for transport readiness, maps native events and reverse requests into the Cypheria protocol, records stderr, and shuts the process down with the Server lifecycle. Desktop never imports `@openai/codex`, launches Codex, or receives raw Codex JSON-RPC.
...
The transport selects Codex, Claude, Pi, OpenCode, or ACP from the Thread's canonical `agentId`; new chats may choose any installed and enabled server-managed Agent. Queued follow-ups are owned by the retained Thread scope instead of the mounted workspace, so navigation does not discard them. Browser-local attachment URLs are embedded before crossing the protocol boundary, and canonical user-message items retain their attachment blocks for history restoration.
