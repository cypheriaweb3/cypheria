...
- **Codex App Server provider（已实现）**：负责捆绑 App Server 所支持的 public、个人、分享、工作区、仓库、Git/npm-backed 和本地 marketplace 的发现与安装。
...
市场发现分别查询所有 App Server 生成类型并在合并重复项时保留来源：`vertical` 对应 public remote directory，另外还有 `workspace-directory`、`shared-with-me`、`created-by-me-remote` 和 `local`。当前目录展示 Public 与 Personal 页签；Personal 按需显示我创建的、与我共享的、本地市场和工作区分组。Repository、Git、npm 与 filesystem transport 由 App Server 解析，不能当作可见范围。部分来源失败时显示错误，不隐藏成功加载的来源。
...
Cypheria Marketplace discovery/trust provider、自动注册其 GitHub repo、技能录制、高级 MCP 编辑及剩余视觉状态仍待实现；真实登录 Electron 授权尚未端到端验证。分类沿用服务端元数据，不虚构热门排名。
