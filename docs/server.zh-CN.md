...
这套基础刻意不暴露 agent、project、wallet、policy 或 automation 产品 API。它只托管一个裸 `CypheriaRuntime`；runtime 内置的 `runtime.info`、`runtime.health` 与 `runtime.services` 足以验证 transport。等 server 架构和安全边界评审后，再注入产品 services。
...
`@cypheria/protocol` 只定义 Cypheria protocol，不复制也不手写 Codex App Server types。所有 HTTP 与 WebSocket boundary value 都使用 Zod 校验。
