...
The Review panel remembers the most recently selected source in Desktop browser storage and falls back to unstaged changes when that source requires a thread that is not available. GitHub PRs can be attached to or detached from a chat; the Desktop persists up to 20 validated PR links per thread in browser storage and shows an attachment icon in the sidebar when the Git setting allows it. Git settings remain in Server configuration. Server records the start and outcome of Git mutations in the shared audit log with the request ID; audit events omit paths, commit messages, PR bodies, and file contents.
