...
Settings uses one flattened, virtualized navigation list. The back row, group labels, ordinary settings items, the expandable Agent harnesses row, and all visible Agent child rows share one scroll container and one TanStack Virtual virtualizer. Search and the theme footer stay outside that container. Expansion, search, installation changes, and registry updates rebuild the flat row model; route changes scroll the active item into view. Agent child rows never introduce a nested scroller or second navigation virtualizer. The Agent harnesses parent is an expand control rather than a page and is never active. Its children always include the four native harnesses and include registry harnesses only while installed.

The add action on the Agent harnesses row opens the uninstalled-harness picker. It displays Server operation progress while installing and opens the new harness settings after completion. Agent harness routes use `/settings/agent-harnesses/$agentId/$sectionId`. The selected Agent owns a second-level section menu for authentication, models, and discovered settings categories; on narrow screens it becomes a selector. Update and uninstall actions live in the Agent header rather than a separate installation section, and uninstalling a registry harness removes it from the navigation. The right panel contains the actual section. Network proxy remains above the Agent header, starts collapsed, and retains its expanded state while the Agent or section changes. Models use a separate fixed-height virtualized list with provider filtering and an explicit Server refresh.
