...
General settings place Permissions above General. Default permissions is always on and is not persisted. Full access availability is saved in the Desktop `config.json` at `composer.permissionModeVisibility`; enabling it requires confirmation and only reveals Full access in the composer menu. Codex permissions and other defaults are saved in the isolated Codex `config.toml` through App Server.
...
The Review panel remembers the most recently selected source in Desktop browser storage and falls back to unstaged changes when that source requires a thread that is not available. GitHub PRs can be attached to or detached from a chat; the Desktop persists up to 20 validated PR links per thread plus attachment history in browser storage, supports reverse lookup from a PR to linked chats, and shows an attachment icon in the sidebar when the Git setting allows it. Git settings remain in Server configuration. Server records the start and outcome of Git mutations in the shared audit log with the request ID; audit events omit paths, commit messages, PR bodies, and file contents.
