...
Composer text and ordered attachment metadata are stored under `composerDraft:<scopeId>` with a 250 ms debounce and page-hide flush. A new-composer scope is placed in route search until the Server creates a Thread, then rebound to `composerDraft:<threadId>`. File-picker inputs use Electron `webUtils.getPathForFile()` and main-process direct copy; only clipboard, pasted-text, screenshot, and other already in-memory sources use the bounded byte path. Failed submissions retain the complete draft. Missing binary data remains visibly unavailable and cannot be submitted.
