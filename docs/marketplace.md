...
`apps/marketplace` is a separate remote trust boundary. It must not import Electron, desktop IPC, the `apps/server` runtime, the Server Codex adapter, or the local SQLite adapter in `@cypheria/db`. Framework-neutral schemas, API contracts, and UI primitives may move to dedicated packages once stable.
