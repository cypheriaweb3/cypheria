...
会话内容区使用 TanStack Virtual，并通过实测的可变高度消息行、稳定 message ID、末端锚定、追加跟随和 overscan 避免长历史对话一次挂载全部 turn。Renderer 维护一个最多 20 项的 thread scope，保存已测量 row 快照、稳定可见消息锚点、原始 offset、距底部距离、视口高度与锁底状态。恢复在绘制前的 layout effect 中完成，能在路由切换与异步 row 测量后保持稳定锚点；新会话或已锁底会话则继续位于真实底部。AI Elements `Conversation` 只提供结构与 context：desktop 注入自己的 instance，禁用上游弹簧/resize 滚动，并关闭浏览器 scroll anchoring，避免与 TanStack 的修正竞争。macOS 关闭主窗口时只隐藏窗口，重新激活会展示同一个 renderer，从而保留 session scope；真正退出仍会销毁它。职责划分与升级约束参见 [AI Elements 集成与升级指南](ai-elements.zh-CN.md)。标题栏支持通过 App Server 原位重命名 thread。对齐 ChatGPT 的输入框使用无边框悬浮 surface、随内容增高并以 `25dvh` 为上限的编辑器、显式左右分组的 footer、经过校验的附件与截图捕获、技能插入、语音输入、项目与权限选择、合并的模型/推理菜单，以及相互独立的提交、停止、运行中 steer 和下一 turn queue 操作。应用包编辑器的剪贴板路由也得到保留：只有图片的粘贴会变成附件，图片加独立文本会保留文本路径，达到 5,000 字符的粘贴会变成由 scope 持有的 `Pasted text.txt` 卡片，且不超过 25,000 字符时可在当前选区恢复。由于 App Server v2 没有通用文件输入，初次消息、运行中 steer 和排队消息的边界都会把这类内联文本附件解码成完整的 `text` user input。共享滚动面保留 macOS 原生 overlay 几何，并使用透明 track 和在 hover/主动滚动时增强的低对比度 thumb。工作区 chrome 采用嵌套的可调整尺寸布局：右侧面板限制在 320 像素到工作台宽度一半之间；底部面板默认 280 像素，并限制在 160 像素到可用高度一半之间。标题栏底部面板控件与 `Cmd/Ctrl+J` 会独立于 `Control+反引号` 终端操作切换这个 dock。隐藏时面板折叠到零，但不会关闭标签或 PTY；重新打开后会恢复仍挂载的 Xterm surface、已有输出和上次的像素高度。关闭最后一个标签后会保留空 dock，并把面板操作改为“关闭”；从隐藏状态打开空 dock 时才会明确新建终端。标签条独立横向滚动，新增标签控件则固定在其右侧，与应用包中的面板 chrome 一致。面板状态与终端 controller 位于带 `key` 的会话 subtree 之上，因此切换会话仍会保留已打开的 dock、PTY、活动标签和高度；会话 subtree 重新挂载时，通过有界 replay buffer 恢复 Xterm 输出。项目范围的 `node-pty` 终端仍可在底部 dock 和右侧标签之间移动而不丢失会话；没有可见终端 dock 时，终端操作才会读取设置中的默认底部/右侧位置。Renderer IPC 只传可选的 App Server project ID；Electron main 在启动 shell 前解析可信项目根目录，并统一拥有终端输入、resize、output、exit 和清理流程。
