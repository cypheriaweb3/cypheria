...
The protocol pins `@anthropic-ai/claude-agent-sdk@0.3.270` and maps every network-safe top-level function, every `Query` control method, serializable query options, and all 40 `SDKMessage` output variants to `agent.claude.*` logical messages. Requests and responses are paired by `requestId`. A client-selected `queryId` identifies a running async iterator, while `warmQueryId` identifies a reusable `startup()` handle. Text prompts are carried directly; `AsyncIterable<SDKUserMessage>` is represented by a stream prompt followed by ordered input and input-complete notifications.
