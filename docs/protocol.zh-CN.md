...
GitLab MR 详情读取、标题更新和普通评论通过已连接 GitLab App 的 `codex_apps` 工具执行，要求关联本地 Codex 线程。Server 校验线程属于请求的仓库、origin 为 GitLab.com、项目和 MR URL 与 origin 一致，且所需工具绑定同一个 connector 账户 link。读取调用后会复核工具 resource URI；写入调用在发送前校验 link，并确认返回结果，不自动重试。其他 GitLab MR 操作仍在计划中。
