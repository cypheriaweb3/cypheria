...
`git.worktree-move-thread.request` 可选择在源与目标 HEAD 相同且目标干净时复制本地改动。`git.synced-branch-state/sync/undo` 支持将托管 detached worktree 的已提交变更受保护地同步到选定的本地分支。同步会拒绝脏工作树或已在外部移动的分支，先前的分支提交保存在 `refs/cypheria/worktree-sync/*`，并更新 checkout 与元数据；撤销要求同步后的分支未再次变化。setup 只捕获少量白名单工具链环境变量的变化，以兼容的 `codex-shell-environment.json` 格式写入 worktree Git 目录，并保留在托管元数据中供恢复。
`git.clone-state.request` 返回浅克隆与部分克隆状态。`git.worktree-starting-ref.request` 将选定分支或修订解析为固定提交。Git config 读写请求只允许操作工作树专属配置中的 `codex.localEnvironmentConfigPath`；启用工作树配置前，读取返回 null。`git.apply-review-sections.request` 按顺序执行最多 100 个固定文件修订的 Review 操作，逐项返回已应用、过期、冲突或失败结果，调用方可保留成功项并只刷新失败项。Review 面板打开时定期重新读取 Git；Server 不保留可能跨越外部文件系统变更的 Git 读取缓存。
...
GitHub App 按操作分别报告列表、账户范围检索、详情、差异、评论及审查、检查、审查线程、媒体和创建能力。可用的每组工具必须属于该仓库选定的同一账户 link。Desktop 只启用受支持的 App 读取；除创建外的 PR 修改由 `gh` 执行。PR 表单可先推送当前分支，并建议已知的默认 base 分支。创建结果不确定时，会刷新 PR 数据并提示用户在重试前查看 GitHub。
