...
GitLab MR 详情通过已连接 GitLab App 的 `codex_apps` 工具读取，要求关联本地 Codex 线程。Server 校验线程属于请求的仓库、origin 为 GitLab.com、项目和 MR URL 与 origin 一致，且所需工具绑定同一个 connector 账户 link。每次调用后还会复核工具 resource URI。其他 GitLab MR 操作仍在计划中。
