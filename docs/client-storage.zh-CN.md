...
Web adapter 把附件 bytes 放在专用 IndexedDB 数据库中。Expo Native 放在应用 document 目录。Desktop 只为内存 source 通过隔离 preload bridge 传输带上限的 `Uint8Array`；file URI source 由 Electron main 直接复制。Main 会校验 key、执行 32 MiB 限制，并且只在自己管理的目录内写文件。Renderer 永远不会获得 Node.js 权限。
