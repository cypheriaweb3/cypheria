...
`@cypheria/protocol` reserves `codex`, `claude`, `pi`, and `opencode` as native IDs and validates other ACP registry IDs by their published shape. New registry entries therefore become usable after a registry refresh without a Cypheria release. The generated registry ID list is an audit snapshot only, not a runtime allowlist. Integrations use the broader compatibility tags `codex`, `claude`, `pi`, `opencode`, and `acp` so one declaration can cover registry Agents.
