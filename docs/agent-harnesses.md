...
## Branching and native lifecycle

The common adapter exposes exact provider branches rather than replaying a summary or old prompts. Codex uses `thread/fork`: a user boundary supplies that user's turn as `beforeTurnId`, a non-final assistant boundary supplies the following turn as `beforeTurnId`, and a final assistant or thread-head omits the boundary. Claude uses `forkSession`; a user boundary selects the preceding native message (the first user message yields a blank session), while an assistant boundary selects that assistant message. OpenCode uses `session.fork`; user messages become `before`, assistant messages use the following user message as `before`, and a final assistant uses a complete fork. Pi switches an independent target runtime to the source session and uses fork-before for a user or non-final assistant boundary; thread-head and a final assistant clone the session. The resulting Pi session-file path becomes the durable harness binding. Every supported first-party branch then reads the target history and rebuilds a fresh Canonical Timeline epoch with stable native message and turn identities.

ACP advertises only thread-head Fork, and only when negotiated `session.fork` exists. ACP has no reliable message-level branch or rollback boundary, so Cypheria does not advertise message Fork or Rewind and never emulates them. A complete ACP fork retains the exact source Canonical Timeline when the protocol surface cannot replay target history.

Native lifecycle mapping is deliberately asymmetric. Archive is local for every Agent; Codex also receives archive/unarchive and OpenCode receives the matching archived session update. Claude, Pi, and ACP remain local-only for archive. Rename synchronizes to Codex, Claude, OpenCode, and Pi after the local title is committed; ACP rename is local. Delete invokes Codex delete, Claude session delete, OpenCode session remove, or capability-gated ACP `session/delete`; Pi deletion remains local because its RPC surface does not provide a sufficiently explicit safe delete. Native archive and rename failures are warnings, while native restore must succeed before local unarchive.
