...
The conversation surface uses TanStack Virtual with measured variable-height message rows, stable message IDs, and overscan so long restored threads do not mount every turn at once. The header supports inline App Server thread renaming. Its composer supports validated attachment and screenshot capture, skill insertion, dictation, project and permission selection, model and reasoning controls, and distinct stop, mid-turn steer, and next-turn queue actions. Workspace chrome is a nested resizable layout: the side panel is constrained to 320 pixels through half the workbench, while the bottom panel defaults to 280 pixels and remains between 160 pixels and half the available height. A project-scoped `node-pty` terminal can move between the bottom dock and a side-panel tab without losing its session. Renderer IPC passes only an optional App Server project ID; Electron main resolves the trusted project root before spawning the shell and owns terminal input, resize, output, exit, and cleanup.
