...
`apps/server` is the only target-architecture process that owns `@cypheria/runtime`. It exposes a small Hono HTTP operations API and a versioned WebSocket session protocol from `@cypheria/protocol`. A supervisor owns the PID lock, bidirectional liveness supervision, bounded crash restart, process-group termination, and graceful shutdown; the replaceable worker owns Hono, logical sessions, runtime lifecycle, and runtime-event broadcasting. Direct sockets and decrypted relay channels enter the same physical-connection boundary. A dropped transport retains its logical session for a bounded grace period so `@cypheria/client` can resume it with the prior session ID and matching client identity. Desired server configuration lives at `$CYPHERIA_HOME/config/server.json`; the worker resolves environment overrides once at startup and exposes desired-versus-running restart state without returning its environment-only authentication token.
...
Cypheria-owned object schemas strip unknown keys. The optional `session.ready.features` record is the
...
notification, and `connection.initialized` exposes the negotiated snapshot. Reverse requests always
terminate with either their typed response or `client.error` while the transport remains available.
