...
`@cypheria/protocol` defines Cypheria client messages, server messages, HTTP bodies, and runtime method validation. Its WebSocket layer follows Paseo: top-level `hello`, `ping`, and `pong` messages handle physical connections, while `{ type: "session", message }` carries all logical server, ACP, and Codex traffic. A logical `server.status.notification` message completes attachment; there is no `session.ready`. WebSocket messages remain plain JSON when their values are JSON-native; a versioned SuperJSON envelope carries metadata only when Cypheria-owned payloads contain values such as `bigint`. The package owns generated Codex App Server TypeScript, JSON Schemas, response mappings, and per-message Zod validators, and exposes the complete provider-transparent Cypheria API as `agent.codex.<operation>.request|response|notification` from its root entry. Raw generated Codex types are isolated behind `@cypheria/protocol/codex-types`. `@cypheria/codex-bridge` consumes these artifacts without maintaining another generated copy and temporarily owns raw Codex JSON-RPC validation until it is refactored around the Cypheria messages. Client code may depend on `@cypheria/protocol`; it must not import server internals or privileged domain implementations. `@cypheria/client` packages this boundary into three layers: `ServerClient` owns a validated WebSocket session, `CypheriaApi` borrows an existing `ServerClient` without lifecycle control, and `CypheriaClient` combines that facade with connection lifecycle. Its `codex` entry implements a Cypheria-owned SDK-shaped `client()` and `ClientApp`; `connect()` and `connectWith()` accept either facade through the shared `CypheriaApi` type and internally select the minimal `agent.codex` endpoint. Typed request calls, reverse-request handlers, and notification handlers then run without a generated action tree. Its stable and draft-v2 ACP entries provide the same app shape and select `agent.acp` from the supplied `CypheriaApi`; they reuse the official handler/JSON-RPC engine and selectively re-export supported SDK types and helpers without deprecated connection classes. This keeps one host-owned connection shareable without allowing a plugin or local surface to close it.

Generated Codex schema names omit the redundant `Message` segment, for example `AgentCodexClientRequestSchema`. Inbound and outbound session schemas nest the Codex and ACP family discriminators inside the session `type` discriminator while keeping every concrete message type directly routable. Cypheria protocol v2 and `cypheria.v2` require every ACP wire message to carry a generated `AgentId` beside `protocolVersion` and `type`; batch outer and inner IDs must match. ACP wire names contain the normalized method and request/response/notification suffix; a separate numeric `protocolVersion` discriminates stable ACP v1 from draft v2. Underscore-prefixed extension methods use dedicated extension message types with `method` nested in `payload`; v2 uses a dedicated batch type whose `payload.messages` is another nested discriminated union. A generated ACP catalog derives the known method pairs and method-specific validators from the pinned SDK declarations, and known params/results use the canonical parsed SDK output.

Claude Agent SDK calls use the same request/response envelope under `agent.claude.*`. A client-selected `queryId` turns the SDK's local `Query` async iterator into a routable stream; `AsyncIterable<SDKUserMessage>` input becomes ordered input notifications, and each concrete SDK output `type`/`subtype` becomes a directly discriminable notification while retaining the original typed value in `payload`. A declaration-driven registry pins the SDK version and inventories its message variants, query methods, options, and exports. Callback-bearing options and function-defined SDK MCP tools stay on the privileged side of the network boundary; `startup()` is also intentionally omitted. `@cypheria/client/claude` reconstructs the SDK-shaped query iterator and network-safe utility calls over a borrowed `CypheriaApi`; the server owns query and process lifecycle per logical session.

Pi's full `pi --mode rpc` contract is exposed under `agent.pi.*`, backed by pinned `@earendil-works/pi-coding-agent@0.85.1` types. Every native command becomes a concrete request/response pair, and every native session event becomes a directly discriminable notification that retains the exact Pi value in `payload`. Blocking extension UI interactions become reverse RPCs; fire-and-forget UI changes remain notifications. Protocol conversion helpers isolate the strict JSONL subprocess adapter, while `@cypheria/client/pi` reconstructs Pi's process-independent `RpcClient` methods over a borrowed `CypheriaApi`. The server owns one Pi process per logical session. See [Pi RPC Protocol](pi-rpc-protocol.md).
...
date. Final protocol unions use explicit Zod AOT compilation, while ACP call validation bypasses the
SDK's broad, uncorrelated params union before applying the selected method schema. Each Codex client
connection owns an initialization state machine; `initialize()` performs the request and initialized
