...
| Claude Code protocol | `agent.claude.*` 下适合网络传输的 `@anthropic-ai/claude-agent-sdk@0.3.270` surface |
| Pi RPC protocol | `agent.pi.*` 下的 `@earendil-works/pi-coding-agent@0.85.1` `pi --mode rpc` surface |
...
`@cypheria/protocol` 使用 Zod author live WebSocket contract，并持有 generated Codex App Server TypeScript、JSON Schema、response mapping 与 validator。完整 `agent.codex.*` RPC 与 notification catalog 从这些已提交产物机械派生；provider payload 在共享 wire 上保持 JSON-transparent。Cypheria 自有 envelope 使用会剥离未知 key 的 object，provider-owned extension surface 会被显式验证。最终大型 message union 显式执行 AOT compile；所有可路由 agent message `type` 都由嵌套 discriminated union 暴露。对象 schema 使用 `.extend()` 或 shape spread 组合；只有上游 JSON Schema 将 variant union 与公共约束组合时，才保留生成的 intersection。它还导出基于官方 SDK 稳定 v1 与显式 draft-v2 type/generated Zod validator 的 `agent.acp.<operation>.request|response|notification` 消息；数字 `protocolVersion` 是 ACP 版本 discriminator。以 underscore 开头的 extension method 与 v2 batch 使用专用 type；batch entry 使用另一层嵌套 discriminated union。Generated ACP catalog 从 SDK handler declaration 派生 51 组 RPC 与 22 个 notification，每个已知 method 都返回选中 params 或 result validator 的规范输出。一个最小且固定版本的 pnpm patch 会暴露 SDK 已发布但私有的 v1/v2 Zod module，无需复制。

同一 package 精确固定 `@anthropic-ai/claude-agent-sdk@0.3.270`，并通过 `agent.claude.*` 暴露选定的网络安全 API。可序列化的 `query()` option、session/settings function、全部 `Query` control method、streaming input 与 40 种输出都有协议表示，无需跨 wire 发送 callback、iterator、process handle 或 session-store object。`startup()`、`tool()` 与 `createSdkMcpServer()` 被明确排除。Generated registry 检查安装包 declaration；SDK 的 `SDKMessage` union、29 个 `Query` method、67 个 option key、17 个 exported function 或 package version 一旦漂移，lifecycle check 就会失败。Runtime SDK type 可通过 `@cypheria/protocol/claude-types` 使用；`@cypheria/client/claude` 提供 SDK-shaped client 门面，server dispatch 仍延后实现。详见 [Claude Agent SDK Protocol](claude-agent-sdk-protocol.zh-CN.md)。

Protocol 还精确固定 `@earendil-works/pi-coding-agent@0.85.1`。完整的 33-command `pi --mode rpc` API 被映射为配对且可直接判别的 `agent.pi.*.request|response` message；23 种 agent/session event、extension error 与全部 9 种 extension UI operation 会在具体 notification 或反向 RPC 中保留原生 typed payload。`@cypheria/protocol/pi-types` 提供 type-only upstream access，转换 helper 则为后续 server adapter 产生精确 JSONL value。`@cypheria/client/pi` 在借用连接上重建不含进程职责的 `RpcClient` command/event API。子进程 start/stop、stderr、executable、environment 与 signal ownership 留在 server。详见 [Pi RPC Protocol](pi-rpc-protocol.zh-CN.md)。

`@cypheria/client` 依赖 `@cypheria/protocol`、只处理传输的 `@cypheria/relay`，以及 protocol 所用的同一精确版本 `@agentclientprotocol/sdk`。它的 `ServerClient` 实现可注入 transport boundary、browser 与 Node WebSocket adapter、hello/authentication、请求关联、超时、protocol validation、typed error、事件分发与有界指数退避重连。`CypheriaApi` 是不带 lifecycle 的借用门面；`CypheriaClient` 持有一条连接。该门面只暴露当前 protocol message family：server status/diagnostics/configuration，以及 typed Codex、ACP 与 Claude traffic；它不会从 runtime method 字符串推断产品 action。`@cypheria/client/codex` 实现 Cypheria 自有的 SDK-shaped `client()` 与 `ClientApp` API：`connect()` 和 `connectWith()` 接收 `CypheriaApi`，在内部选择 `agent.codex`，并为每次 request/response exchange 暴露 typed async `ClientContext.request()`。fluent handler 接收 notification 与反向 request，并写回 typed reverse response，不使用 generated action tree。稳定的 `@cypheria/client/acp` 与显式的 `@cypheria/client/acp/v2` 入口使用相同的 `CypheriaApi` connection 形态，选择性重新导出支持的 upstream helper 与 type，并省略已废弃 connection API。其私有的有方向 Web Streams bridge 把 SDK JSON-RPC traffic 转换成 protocol-owned 逻辑消息，通过专用嵌套 batch 消息保留 v2 batch，使用 connection-local request-ID 状态恢复 response type，并在 Cypheria transport 断开时终止 pending ACP work。`@cypheria/client/claude` 把借用的 `CypheriaApi` 绑定为 SDK-shaped 门面：`query()` 是 async iterator，control/session/settings method 保持上游调用形态，本地 abort 或 transport failure 只终止对应的远程 query。Codex app 同样在每个 endpoint 上只允许一个活跃 consumer，并在 transport loss 时关闭。
