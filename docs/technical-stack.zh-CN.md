...
| Primitive layer | overlays 和 interactive primitives 使用 Base UI；不使用 Radix UI |
| Styling | Cypheria CSS tokens 与 class variants；如后续需要可再加入 Tailwind |
| AI UI reference | OpenAI Apps SDK UI、ai-elements patterns |
| Icons | lucide-react |
| Motion | motion |
| Toast | Base UI Toast baseline |
...
- 使用 shadcn 的 copied-component model 和 styling conventions，但 overlays 与 interactive primitives 优先使用 Base UI，而不是 Radix UI。
- `@cypheria/ui` 当前暴露 Button、Input、Dialog/Sheet、Sidebar shell、Tooltip、Badge 和 Toast primitives。Dialog、Sheet、Tooltip 和 Toast 基于 Base UI；Button、Input、Badge 和 Sidebar 使用轻量 Cypheria wrappers 与 shared CSS tokens。
...
- 当 Base UI 已提供 primitive behavior 时，不重写 Dialog、Dropdown、Popover、Toast、Tooltip 等基础组件。
