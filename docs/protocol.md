...
When a Thread belongs to a Project, its normalized `cwd` must exactly match one of the Project's saved roots; omitting `cwd` selects the first, primary root. The persistence boundary enforces this invariant when a Thread is created, moved into a Project, or changes `cwd`, and prevents a Project update from removing a root still used by a member Thread. Codex receives the complete ordered root list as `runtimeWorkspaceRoots` on start, fork, resume, and every turn start, but Cypheria does not create Codex projects, send a native project ID, or change Codex Thread project metadata. Claude receives `options.additionalDirectories`, and ACP Agents that advertise `session.additionalDirectories` receive `additionalDirectories` on session new, fork, load, or resume; for both, this list contains the remaining Project roots after the current `cwd` is removed and is omitted when empty. OpenCode receives only the resolved `cwd`: create uses `session.create.location.directory`, while fork and resume use `session.move` to align the session directory. Cypheria sends no project metadata to OpenCode and does not create or persist an OpenCode project mapping. Pi also has no additional-directory surface, so its per-Thread RPC process is started with the resolved `cwd`.
