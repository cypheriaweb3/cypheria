...
The `git` capability provides local repository discovery and initialization, status, branch listing, branch creation and checkout, diff, stage, unstage, commit, and push through the Server Git executor. Each `git.*.request` returns a correlated typed response with a success value or error. Git operations use the Server host's filesystem and Git installation.
