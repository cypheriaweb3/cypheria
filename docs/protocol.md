...
GitLab MR detail, title updates, and ordinary comments use the connected GitLab App's `codex_apps` tools for a local Codex thread. Server checks that the thread belongs to the requested repository, its origin is GitLab.com, the project and MR URLs match that origin, and the required tools share one connector account link. It rechecks the tool resource URI after read calls; write calls validate the link before sending and confirm the response without retrying. Other GitLab MR actions remain planned.
