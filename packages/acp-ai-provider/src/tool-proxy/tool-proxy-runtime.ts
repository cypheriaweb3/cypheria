...
/* MCP stdio handshake versions supported by @modelcontextprotocol/sdk 1.x. */
const isDeno=typeof Deno!=="undefined",MCP_VERS=["2025-11-25","2025-06-18","2025-03-26","2024-11-05","2024-10-07"];
let tools=[],host=null,pending=new Map,buf="",hbuf="",rid=0;
const env=k=>isDeno?Deno.env.get(k):process.env[k];
const token=env("ACP_TOOL_PROXY_TOKEN")||"";
const write=async m=>{
...
const init=async()=>{
  ...
  if(p){
    ...
    try{tools=await hostReq("getTools",{token})}catch(e){
...
const onHostMsg=async l=>{try{let r=JSON.parse(l),h=pending.get(r.id);
  if(h){pending.delete(r.id);r.error?h.reject(new Error(r.error.message)):h.resolve(r.result)}
  else if(r.method==="toolsChanged"){tools=await hostReq("getTools",{token});await write({jsonrpc:"2.0",method:"notifications/tools/list_changed"})}}catch{}};
const handle=async l=>{let m;try{m=JSON.parse(l)}catch{return}if(m.jsonrpc!=="2.0")return;
  if(m.method==="initialize"){let requested=m.params&&m.params.protocolVersion;let version=MCP_VERS.includes(requested)?requested:MCP_VERS[0];send(m.id,{protocolVersion:version,capabilities:{tools:{listChanged:true}},serverInfo:{name:"cypheria-acp-tool-proxy",version:"1.0"}})}
    ...
    else try{let r=await hostReq("callHandler",{token,name:p.name,args:p.arguments||{},toolCallId:String(m.id)});send(m.id,r)}
