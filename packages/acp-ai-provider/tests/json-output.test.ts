...
Deno.test("createJsonCleanupTransform", async (t) => {
  ...
  function collectTextDeltas(parts: LanguageModelV4StreamPart[]): string[] {
  ...
  await t.step("preserves incremental streaming for larger JSON payloads", async () => {
    const parts: LanguageModelV4StreamPart[] = [
      { type: "text-start", id: "t1" },
      { type: "text-delta", id: "t1", delta: '```json\n{"name":"Tok' },
      {
    ...
    assertEquals(textDeltas.join(""), '{"name":"Tokyo","country":"Japan","population":14000000}')
  })
  ...
  await t.step("handles fence prefix split across chunks", async () => {
    ...
    const result = await collectStream(parts)
    assertEquals(collectTextDeltas(result).join(""), '{"city":"Tokyo","country":"Japan"}')
  })
