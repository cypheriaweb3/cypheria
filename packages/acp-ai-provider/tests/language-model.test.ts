...
import { ACPLanguageModel } from "../src/language-model.js"
import { createACPProvider } from "../src/provider.js"
import type { ACPProviderSettings } from "../src/types.js"
import { assertEquals, assertExists, assertRejects } from "./assert.js"
import { Deno } from "./deno-compat.js"

// Helper to create minimal provider settings
function createProviderSettings(overrides: Partial<ACPProviderSettings> = {}): ACPProviderSettings {
  return {
    command: "gemini",
...
Deno.test("ACPLanguageModel - implements LanguageModelV4 interface", () => {
  const model = new ACPLanguageModel("test-agent", undefined, createProviderSettings())

  // Check required LanguageModelV4 properties
  assertEquals(model.modelId, "test-agent")
  assertEquals(model.specificationVersion, "v4")
  assertEquals(model.provider, "acp")
  assertEquals(typeof model.doGenerate, "function")
  assertEquals(typeof model.doStream, "function")
})

Deno.test("ACPProvider returns independent model instances", () => {
  const provider = createACPProvider(createProviderSettings())
  const first = provider.languageModel("first")
  const second = provider.languageModel("second")
  assertEquals(first === second, false)
  assertEquals(first.modelId, "first")
  assertEquals(second.modelId, "second")
  provider.cleanup()
})

Deno.test("ACPLanguageModel advertises native HTTP URL support", () => {
  const model = new ACPLanguageModel("test-agent", undefined, createProviderSettings())
  assertEquals(model.supportedUrls["*/*"]?.[0]?.test("https://example.com/a.pdf"), true)
})

Deno.test("ACPLanguageModel - works directly with AI SDK 7 generateText", async () => {
  const model = new ACPLanguageModel("test-agent", undefined, createProviderSettings())
  let updateHandler: ((notification: unknown) => void) | undefined

  ;(
    model as unknown as {
      ensureConnected: () => Promise<void>
      sessionId: string
      client: { setSessionUpdateHandler: (handler: (notification: unknown) => void) => void }
      cleanup: () => void
    }
  ...
  ;(
    model as unknown as {
      promptWithLazyAuthRetry: () => Promise<{ stopReason: string }>
    }
  ...
  assertEquals(result.text, "hello from ACP")
  assertEquals(model.specificationVersion, "v4")
})
...
Deno.test("ACPLanguageModel - maps ACP stop reasons to AI SDK finish reasons", async () => {
  ...
  ;(
    model as unknown as {
      ensureConnected: () => Promise<void>
      promptWithLazyAuthRetry: (request: unknown) => Promise<{ stopReason: string }>
      cleanup: () => void
    }
  ).ensureConnected = () => {
    ;(
  ...
  assertEquals(finishReasons, ["stop", "length", "length", "content-filter", "other"])
})

Deno.test("ACPLanguageModel - closes LanguageModelV4 tool input before emitting a tool call", () => {
  const model = new ACPLanguageModel("test-agent", undefined, createProviderSettings())
  const parts: unknown[] = []
  ...
  ;(
    model as unknown as {
      handleStreamNotification: (
        controller: { enqueue: (part: unknown) => void },
...
function setupConfigOptionModel() {
  ...
  ;(model as unknown as { connection: unknown }).connection = {
    setSessionConfigOption: (request: unknown) => {
      ...
      return Promise.resolve({
        configOptions: [{ ...initialOptions[0], currentValue: "high" }, initialOptions[1]],
      })
    },
  }
...
Deno.test("setConfigOption forwards arbitrary IDs and refreshes config options", async () => {
  ...
  const response = await model.setConfigOption("agent-defined-id", "agent-defined-value")
  ...
  assertEquals(model.getConfigOptions("thought_level")[0].currentValue, "high")
})
...
Deno.test("setThoughtLevel resolves the agent-advertised config ID by category", async () => {
  ...
  assertEquals(model.getConfigOptions("_style")[0].id, "agent-specific-style-id")
})
...
Deno.test("setConfigOptionByCategory rejects missing and ambiguous categories", async () => {
  ...
  ;(model as unknown as { sessionResponse: unknown }).sessionResponse = {
    sessionId: "session-1",
    configOptions: [...options, { ...options[0], id: "second-reasoning-id" }],
  }
...
Deno.test("setModel routes through the agent's session config option", async () => {
  ...
  assertEquals(model.getConfigOptions("model")[0].currentValue, "gemini-3.7-flash")
})
...
Deno.test("setModel validates the value against the advertised model option", async () => {
  ...
  ;(model as unknown as { connection: unknown }).connection = {
    setSessionConfigOption: () => Promise.resolve({ configOptions: [modelOption] }),
  }

  await assertRejects(() => model.setModel("gemini-4"), Error, "is not available")
})

Deno.test("setModel rejects the removed legacy model API", async () => {
  const model = new ACPLanguageModel("test-agent", undefined, createProviderSettings())

  ;(model as unknown as { sessionId: string }).sessionId = "session-1"
  ;(model as unknown as { sessionResponse: unknown }).sessionResponse = { sessionId: "session-1" }
  ;(model as unknown as { connection: unknown }).connection = {}

  await assertRejects(
    () => model.setModel("haiku"),
    Error,
    "did not advertise a session config option"
  )
})
...
Deno.test("setModel rejects ambiguous or missing model config options", async () => {
  ...
  ;(model as unknown as { sessionResponse: unknown }).sessionResponse = {
    sessionId: "session-1",
    configOptions: [modelOption, { ...modelOption, id: "secondary-model" }],
  }
...
Deno.test("setMode routes through the agent's session config option", async () => {
  ...
  ;(model as unknown as { connection: unknown }).connection = {
    setSessionConfigOption: (request: unknown) => {
      ...
      return Promise.resolve({
        configOptions: [{ ...modeOption, currentValue: (request as { value: string }).value }],
      })
    },
  ...
  assertEquals(model.getConfigOptions("mode")[0].currentValue, "plan")
})
...
Deno.test("setMode validates the value against the advertised mode option", async () => {
  ...
  ;(model as unknown as { connection: unknown }).connection = {
    setSessionConfigOption: () => Promise.resolve({ configOptions: [modeOption] }),
  }

  await assertRejects(() => model.setMode("ask"), Error, "is not available")
})
...
Deno.test("setMode rejects ambiguous or missing mode config options", async () => {
  ...
  ;(model as unknown as { sessionResponse: unknown }).sessionResponse = {
    sessionId: "session-1",
    configOptions: [modeOption, { ...modeOption, id: "secondary-mode" }],
  }
  ...
  await assertRejects(() => model.setMode("plan"), Error, 'no mode option with category "mode"')
})

Deno.test("startSession re-applies the configured model after session re-creation", async () => {
  const model = new ACPLanguageModel("test-agent", undefined, createProviderSettings())
  model.setRequestedModelId("gemini-3.7-flash")
  const modelOption = createModelConfigOption("gemini-3.7-flash-high")
  const requests: unknown[] = []
  ...
  ;(model as unknown as { currentModelId: string | null }).currentModelId = "gemini-3.7-flash"
...
function setupMockModel() {
  ...
  ;(model as unknown as { ensureConnected: () => void }).ensureConnected = () => {
    events.push("connect")
  }
  ;(
    model as unknown as {
  ...
  ).promptWithLazyAuthRetry = (request: { sessionId: string; prompt: unknown }) => {
    promptCount++
...
Deno.test("doStream - normal stream is unaffected by abort handling", async () => {
  ...
  assertExists(finish)
  assertEquals((finish as { finishReason: { raw: string } }).finishReason.raw, "end_turn")
})

Deno.test("doStream - prompt errors emit an error and close the provider stream", async () => {
  const h = setupMockModel()
  const { stream } = await h.model.doStream(basicOptions as never)
  h.promptDeferreds[0].reject(new Error("prompt failed"))
  ...
  assertEquals((parts[1] as { error: Error }).error.message, "prompt failed")
})

Deno.test("doStream - raw ACP updates honor includeRawChunks", async () => {
  const create = async (includeRawChunks: boolean) => {
    const h = setupMockModel()
  ...
  assertEquals(
    (await create(false)).some((part) => (part as { type: string }).type === "raw"),
    false
  )
  assertEquals(
    (await create(true)).some((part) => (part as { type: string }).type === "raw"),
    true
  )
})

Deno.test("tool_call_update does not emit duplicate LanguageModelV4 tool calls", () => {
  const model = new ACPLanguageModel("test-agent", undefined, createProviderSettings())
  const parts: Array<{ type: string }> = []
  const handler = (
    model as unknown as {
      handleStreamNotification: (
        controller: { enqueue: (part: { type: string }) => void },
        notification: unknown
      ) => void
    }
  ).handleStreamNotification.bind(model)
  const controller = { enqueue: (part: { type: string }) => parts.push(part) }
  ...
  assertEquals(parts.filter((part) => part.type === "tool-call").length, 1)
})

Deno.test("generateText registers executable acpTools for doGenerate", async () => {
  const model = new ACPLanguageModel("test-agent", undefined, createProviderSettings())
  let registered: Array<{ name: string; execute?: unknown }> | undefined
  ;(
    model as unknown as {
      ensureConnected: (tools?: Array<{ name: string; execute?: unknown }>) => Promise<void>
      sessionId: string
      client: { setSessionUpdateHandler: (handler: (notification: unknown) => void) => void }
      cleanup: () => void
    }
  ).ensureConnected = async (tools) => {
    registered = tools
    ;(model as unknown as { sessionId: string }).sessionId = "session-1"
    ;(model as unknown as { client: unknown }).client = { setSessionUpdateHandler: () => {} }
  }
  ;(
    model as unknown as {
      promptWithLazyAuthRetry: () => Promise<{ stopReason: string }>
    }
  ).promptWithLazyAuthRetry = async () => ({ stopReason: "end_turn" })
  ;(model as unknown as { cleanup: () => void }).cleanup = () => {}
  ...
  assertEquals(typeof registered?.[0]?.execute, "function")
})
...
Deno.test("doStream - aborted turn fully drains before the next prompt is issued", async () => {
  ...
  assertEquals((finish2 as { finishReason: { raw: string } }).finishReason.raw, "end_turn")
