...
const asBytes = (raw: string | Uint8Array | ArrayBuffer): Uint8Array => {
  if (raw instanceof Uint8Array) return raw
  if (raw instanceof ArrayBuffer) return new Uint8Array(raw)
  throw new TypeError("Expected a binary protocol frame")
}

const parseSent = (
  raw: string | Uint8Array | ArrayBuffer
): ClientMessage | { type: "hello" } | { type: "ping" } => {
  const envelope = decodeWSInboundMessage(asBytes(raw))
...
const sendSession = (socket: TestWebSocket, message: unknown): void => {
  socket.message(encodeProtocolMessage(wrapServerSessionMessage(message as ServerMessage)))
}

const acceptSocket = (socket: TestWebSocket): void => {
  socket.open()
  const hello = decodeWSInboundMessage(asBytes(socket.sent.at(-1) ?? ""))
...
describe("ServerClient", () => {
  it("connects from a relay offer without exposing credentials or protocol messages", async () => {
    ...
    socket.open()
    await tick()
    const e2eeHello = JSON.parse(String(socket.sent[0] ?? "")) as { key: string; type: string }
    ...
    socket.message(JSON.stringify({ type: "e2ee_ready" }))
    await tick()
    const encryptedHello = socket.sent[1]
    if (!encryptedHello) throw new Error("Expected encrypted hello")
    if (!(encryptedHello instanceof ArrayBuffer)) throw new Error("Expected binary ciphertext")
    expect(new TextDecoder().decode(encryptedHello)).not.toContain("client-relay")
    const hello = decodeWSInboundMessage(
      new Uint8Array(decrypt(directionalKeys.clientToServer, encryptedHello))
    )
    expect(hello).toMatchObject({ clientId: "client-relay", type: "hello" })
    const status = encodeProtocolMessage(
      wrapServerSessionMessage({
        payload: { ...serverStatus, capabilities: [] },
        type: "server.status.notification",
      })
    )
    const encryptedStatus = encrypt(directionalKeys.serverToClient, status.slice().buffer)
    socket.message(encryptedStatus)
    await connected
  ...
  it("uses the top-level hello and exposes server status", async () => {
    ...
    const hello = decodeWSInboundMessage(asBytes(socket.sent[0] ?? ""))
  ...
  it("reconnects with the same top-level client identity", async () => {
    ...
    replacement.open()
    const hello = decodeWSInboundMessage(asBytes(replacement.sent.at(-1) ?? ""))
  ...
  it("rejects text protocol frames and times out a stalled handshake", async () => {
